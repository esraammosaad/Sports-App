...
class NetworkService{
   ...
   static func getFootballLeagues (completion : @escaping (LeagueResponse?) -> Void){
    ...
    static func getBasketballLeagues (completion : @escaping (LeagueResponse?) -> Void){
     let url =  "\(Strings.BASEURL)\(Strings.BASKETBALL_ENDPOINT)/?APIkey=\(Strings.API_KEY)&met=\(Strings.LEAGUES_ENDPOINT)"
         AF.request(url).validate(statusCode: 200..<300).responseDecodable(of:LeagueResponse.self){
             response in
             switch response.result{
             case .failure(let error):
     ...
     static func getBasketballLeagueDetails (leagueID : Int,completion : @escaping (EventResponse?) -> Void){
     let url =  "\(Strings.BASEURL)\(Strings.BASKETBALL_ENDPOINT)/?APIkey=\(Strings.API_KEY)&met=\(Strings.EVENTS_ENDPOINT)&from=2025-01-01&to=2025-06-30&leagueId=\(leagueID)"
    ...
    static func getBasketballLeagueTeams (leagueID : Int ,completion : @escaping (TeamResponse?) -> Void){
    let url =  "\(Strings.BASEURL)\(Strings.BASKETBALL_ENDPOINT)/?APIkey=\(Strings.API_KEY)&met=\(Strings.TEAMS_ENDPOINT)&leagueId=\(leagueID)"
        AF.request(url).validate(statusCode: 200..<300).responseDecodable(of:TeamResponse.self){
            response in
            switch response.result{
            case .failure(let error):
